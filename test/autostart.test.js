...
test('unpackaged: runs electron.exe against the app dir with --hidden', () => {
  ...
  const ctx = { isPackaged: false, execPath: 'C:\\electron\\electron.exe', appPath: 'C:\\repo\\gauge' };
  assert.deepStrictEqual(loginItemSettings(cfg, ctx), {
    openAtLogin: true,
    path: 'C:\\electron\\electron.exe',
    args: ['"C:\\repo\\gauge"', '--hidden'],
  });
});

test('unpackaged: quotes an appPath containing a space so it survives as one argv token', () => {
  const cfg = { autoStart: true };
  const ctx = { isPackaged: false, execPath: 'C:\\electron\\electron.exe', appPath: 'C:\\Users\\Some One\\repo\\gauge' };
  assert.deepStrictEqual(loginItemSettings(cfg, ctx), {
    openAtLogin: true,
    path: 'C:\\electron\\electron.exe',
    args: ['"C:\\Users\\Some One\\repo\\gauge"', '--hidden'],
  });
});

test('packaged: just --hidden, no explicit path/exe args', () => {
  const cfg = { autoStart: true };
  const ctx = { isPackaged: true, execPath: 'C:\\Program Files\\Gauge\\Gauge.exe', appPath: 'C:\\Program Files\\Gauge\\resources\\app.asar' };
  assert.deepStrictEqual(loginItemSettings(cfg, ctx), {
    openAtLogin: true,
    args: ['--hidden'],
  });
